...
void init_test(vector<Student_info>& students, vector<Student_info>::size_type n);
...
void init_test(vector<Student_info>& students, vector<Student_info>::size_type n)
{
	for (vector<Student_info>::size_type i = 0; i < n; i++){
		students[i].name.assign("a");
		students[i].midterm = random_num();
		students[i].final = random_num();
		
		for (vector<double>::size_type j = 0; j < 4; j++){
			students[i].homework.push_back(random_num()); 
		}
	}
}
